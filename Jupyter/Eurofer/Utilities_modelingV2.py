...
alpha_default = 1
mu_default = 0
def weibull(x, gamma=gamma_default, alpha=alpha_default, mu=mu_default):
...
def get_model_fit_and_print_it(x, y, fit_func='poly', method='leastsq', fit_fun_args_and_initials={}, 
    ...
    if fit_func == 'poly':
        ...
        default_args = {'gamma': gamma_default, 'alpha': alpha_default, 'mu': mu_default}
        
        for arg, initial in fit_fun_args_and_initials.items():
            if arg == 'gamma':
                params.add('gamma', value=initial, min=1e-10, max=1e3)
            elif arg == 'alpha':
                params.add('alpha', value=initial, min=1e-10, max=1e3)
            else: # var == 'mu':
                params.add('mu', value=initial)

        for arg, initial in default_args.items():
            if arg not in fit_fun_args_and_initials:
                if arg == 'gamma':
                    params.add('gamma', value=gamma_default, vary=False)
                elif arg == 'alpha':
                    params.add('alpha', value=alpha_default, vary=False)
                else: # var == 'mu':
                    params.add('mu', value=mu_default, vary=False)
    ...
    if print_bool:
        ...
        if fit_func == 'poly':
            ...
            display(N(Poly([result.params[alphabet_list[i]].value for i in range(0, poly_deg+1)],\
            ...
            T = Symbol("T")
            display(N((gamma_fit / alpha_fit) * ((T - mu_fit) / alpha_fit)**(gamma_fit - 1) * sympy.exp(-((T - mu_fit) / alpha_fit)**gamma_fit), eq_digits))
