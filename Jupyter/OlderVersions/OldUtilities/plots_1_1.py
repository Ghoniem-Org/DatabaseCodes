...
def data_plot(x_data_list, y_data_list, x_fit_list=None, y_fit_list=None,
    ...
    if x_data_list is not None and y_data_list is not None:
        ...
        for i, (x_data, y_data) in enumerate(zip(x_data_list, y_data_list)):
            plt.scatter(x_data, y_data, label=data_labels[i], color=data_colors[i], s=data_marker_sizes[i], marker = next(marker_cycle))
...
def plot_data(x_data_list, y_data_list, x_for_fit_plot, fit_result, font_size, data_marker_sizes, x_label, y_label, x_lim, y_lim, data_labels, title, legend_loc, legend_font_size, scale = False):
